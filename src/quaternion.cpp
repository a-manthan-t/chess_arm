...
namespace quaternion {
    ...
    std::string Quaternion::show() const { // Components displayed to 4 d.p.
        auto r4 = [](float v) { return std::round(v * 10'000) / 10'000; };
        return std::format("(w: {}, x: {}, y: {}, z: {})", r4(w), r4(x), r4(y), r4(z));
    }

    // Extract the angle rotated around each axis from the quaternion, assuming it
    // is not a vector (the returned structure does not actually represent a quaternion).
    Quaternion Quaternion::toEuler(bool degrees = true) const {
        using namespace std;
